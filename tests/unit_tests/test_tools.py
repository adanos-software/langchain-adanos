...
from langchain_adanos.tools import OPERATIONS, AdanosToolInput
...
def test_every_operation_uses_official_sdk_route(tool, source, operation):
    ...
    with patch.object(httpx.Client, "send", autospec=True, side_effect=send) as request:
        result = tool.invoke(
            {"source": source, "operation": operation, "parameters": params}
        )
...
def test_configuration_and_agent_schema(monkeypatch):
    ...
    with patch.object(httpx.Client, "send") as request:
        assert (
            AdanosMarketSentimentTool().invoke(
                {"source": "reddit", "operation": "trending"}
            )["error"]
            == "configuration_error"
        )
    request.assert_not_called()
...
def test_missing_sdk_has_actionable_error(tool):
    with patch.dict(sys.modules, {"adanos": None}):
        with pytest.raises(ImportError, match="langchain-adanos"):
            tool.invoke({"source": "reddit", "operation": "trending"})
...
def test_timeout_closes_client_without_retry(tool):
    with (
        ...
        result = tool.invoke({"source": "reddit", "operation": "trending"})
